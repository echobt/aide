...
export function MenuBar(props: MenuBarProps = {}) {
  ...
  const handleOpenFolder = async () => {
    try {
      ...
      if (selected) {
        ...
        recentProjectsCtx?.addProject(folderPath);
        // Open in a new window instead of replacing current directory
        await invoke("create_new_window", { path: folderPath });
      }
    } catch (e) {
  ...
  const handleOpenRecentProject = (project: RecentProject) => {
    // Open in a new window instead of replacing current directory
    recentProjectsCtx?.openProject(project, true);
    setActiveMenu(null);
  ...
  const menuActions = {
    ...
    settingsOpen: createMenuAction("settings:open"),
  ...
  const menus = createMemo<MenuSection[]>(() => [
    ...
    },
    {
      ...
      items: [
        ...
        { label: "Replace in File", shortcut: "Ctrl+H", action: menuActions.searchReplaceInFile },
        { separator: true },
  ...
  return (
    <div 
      ...
      style={{
        ...
        "z-index": "2500",
        isolation: "isolate",
        // Reserve space for fixed window controls (3 buttons × 46px = 138px)
        "padding-right": "138px",
      }}
    >
...
onMouseEnter={(e) => {
      ...
      <div 
        class="flex-1 h-full flex items-center justify-center pointer-events-none" 
        data-tauri-drag-region
      >
