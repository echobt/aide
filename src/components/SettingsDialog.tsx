    ...
    <Show when={shouldShow()}>
      <div class="settings-tree-node">
        <UIButton
          onClick={handleClick}
          variant="ghost"
          class={`settings-tab-button ${props.activeSection === props.item.id ? "settings-tab-button-active" : ""} ${hasModifications() ? "has-modifications" : ""}`}
          style={{ 
            "padding-left": `${props.depth * 12 + 12}px`,
            "display": "flex",
            "align-items": "center",
            "width": "100%",
            "text-align": "left",
        ...
        >
          <span class="w-4 h-4 flex items-center justify-center shrink-0">
            <Show when={hasChildren()}>
              <Show when={isExpanded()} fallback={<Icon name="chevron-right" class="h-3 w-3" />}>
                <Icon name="chevron-down" class="h-3 w-3" />
              </Show>
            </Show>
          </span>
          <Show when={props.item.icon}>
            <props.item.icon class="h-4 w-4 shrink-0" />
          </Show>
