 ...
 * - OptimizedProviders handles ALL context providers with flat ProviderComposer
 * - Lazy components gated with Show - only load when visible/needed
 * - Suspense boundaries around component groups
 * - Deferred MCP initialization via requestIdleCallback
 * - Expected improvement: ~11s -> ~4s startup
 */
...
const DocumentSymbolPicker = lazy(() => import("@/components/DocumentSymbolPicker").then(m => ({ default: m.DocumentSymbolPicker })));
  ...
  onMount(async () => {
    unlisten = await listen<DeepLinkAction>("deep-link", async (event) => {
      ...
      switch (action.type) {
        ...
        case "OpenSettings": {
          window.dispatchEvent(new CustomEvent("settings:open", { detail: { section: action.payload.section } }));
          break;
        }
        case "Unknown": {
  ...
  return (
    <ErrorBoundary fallback={(err) => (
      <div class="h-screen w-screen flex flex-col items-center justify-center bg-[#1e1e1e] text-white p-8">
        <h1 class="text-xl font-bold mb-4 text-red-500">Application Crash</h1>
        <pre class="bg-black/50 p-4 rounded text-xs max-w-2xl overflow-auto border border-white/10">
          {err.toString()}{"\n\n"}{err.stack}
        </pre>
        <button onClick={() => window.location.reload()} class="mt-6 px-4 py-2 bg-blue-600 hover:bg-blue-700 rounded">
          Reload
        </button>
      </div>
    )}>
