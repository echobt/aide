...
| `git` | `src/git/` | 24 files: `bisect`, `branch`, `cache`, `cherry_pick`, `clone`, `command`, `diff`, `hunk`, `lfs`, `lines`, `log`, `merge`, `rebase`, `remote`, `staging`, `stash`, `status`, `submodule`, `tag`, `types`, `watcher`, `worktree`, `helpers` | Full Git operations via libgit2 |
